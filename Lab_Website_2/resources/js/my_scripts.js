...
function viewStudentStats(id,toggle){
	...
	if(toggle==0)
	{
		...
		document.getElementById(id).style.hight="0px";
	}
	else
	{
		document.getElementById(id).style.visibility="visible";
		document.getElementById(id).style.hight="auto";
	}
}
...
function loadStatsPage(){
	...
	winsum=0;
	losesum=0;
 	for(row_c=0;row_c<table.rows.length;row_c++)
	{
		for(col_c=0;col_c<table.rows[row_c].cells.length;col_c++)
		{	
			if(row_c>1)
				{
				if(col_c==2)
					{
					cell_v= table.rows[row_c].cells[col_c].innerHTML;
					winner = parseInt(cell_v);
					}
				if(col_c==3)
					{
					cell_v= table.rows[row_c].cells[col_c].innerHTML;
					winner = winner-parseInt(cell_v);
					}
				if(col_c==4)
					{
					if(winner>0)
						{
					table.rows[row_c].cells[col_c].innerHTML="Buffs";
						winsum=+1;
						}
					if(winner==0)
					table.rows[row_c].cells[col_c].innerHTML="N/A";
					if(winner<0)
						{
					table.rows[row_c].cells[col_c].innerHTML="Away Team";
						losesum=+1;
						}
					}
				}
